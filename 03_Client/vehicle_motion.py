...
class HUD(object):
    ...
    def tick(self, world, clock, player):
        ...
        self._info_text += [
            'Violate stop sign: % 4d' % self.crossRTL
            ]
        self._info_text += [
            'Violate limit speed: % 4d' % self.crossRTL
            ]
        self._info_text += [
            'Violate direct sign: % 4d' % self.crossRTL
            ]
        self._info_text += [
            'Score: % 4d' % self.score
            ]
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == K_BACKSPACE:
                    ...
                    self._parent.vehicle_controller.actor_list.append(vehicle)
